...
export default function EditEvent() {
  ...
  const { modalEditEvent, detailEvent } = useSelector((state) => state.event);
  ...
  useEffect(() => {
    if (detailEvent?.id) {
      setName(detailEvent?.name || "");
      setLocation(detailEvent?.location || "");
    }
  }, [detailEvent]);

  useEffect(() => {
    if (modalEditEvent === false) {
      dispatch(setDetailEvent(null));
    }
  }, [dispatch, modalEditEvent]);
  ...
  const handleEditEvent = (e) => {
    e.preventDefault();
    setLoading(true);
    dispatch(editEvent(detailEvent?.id, { name, location })).finally(() =>
      setLoading(false)
    );
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 font-Madimi">
      <div className="bg-white rounded-xl p-4 w-full max-w-lg shadow-lg relative m-4">
        <button
          ...
          onClick={() => dispatch(setModalEditEvent(false))}
        >
          &times;
        </button>
